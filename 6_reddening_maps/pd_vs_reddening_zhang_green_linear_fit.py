...
def add_linear_fit(ax, df, x_col="ext", y_col="P[%]", color="black"):
    """Plot an unweighted linear fit y = m x + b."""
    df = df[[x_col, y_col]].dropna()
    if len(df) < 2:
        return
    m, b = np.polyfit(df[x_col], df[y_col], 1)
    xfit = np.linspace(df[x_col].min(), df[x_col].max(), 100)
    yfit = m * xfit + b
    ax.plot(xfit, yfit, "--", color=color, lw=1.5,
            label=fr"linear fit: $p={m:.1f}A_0{b:+.2f}$", zorder=4)
...
combined_good = pd.concat([rob_good, pan_good], ignore_index=True)
add_linear_fit(ax_all, combined_good)
ax_all.set_title("Combined", fontsize=11)
ax_all.legend(fontsize=8, framealpha=0.7)
format_ax(ax_all)
ax_all.set_xlim(0.0, 0.3)
