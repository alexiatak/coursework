...
def segments_on_map(fitsfile, ra, dec, Ps, pas,
    ...
    #Markkanen cloud — galactic frame
    hp.gnomview(mapread, rot=[MAP_ROT_L, MAP_ROT_B], min=-0.03, max=0.03, cmap='magma',
                xsize=150, ysize=150, fig=2,
                coord='G', reso=11, title="",
                unit=r"$\Delta E(B-V)$", format="%.2g",
                notext=True)
    hp.graticule(dpar=5, dmer=5, alpha=0.55, color="white", lw=0.6)
    #hp.graticule()

    # ------------------------------------------------------------
    # Manual Galactic coordinate labels on the top and left frame
    # ------------------------------------------------------------
    fig_cur = plt.gcf()
    ax = plt.gca()
    ax_bbox = ax.get_position()

    map_span_deg = 150 * 11 / 60.0
    half_deg = map_span_deg / 2.0

    # Coordinate labels to show on the frame.
    # Keep only the central coordinates and one extra coordinate on each axis.
    # In healpy gnomview, Galactic longitude increases to the left.
    l_center = MAP_ROT_L      # 265.0 deg
    l_extra  = 274.0          # extra longitude label

    b_center = MAP_ROT_B      # 80.0 deg
    b_extra  = 69.0           # extra latitude label

    # Top longitude labels.
    y_top = ax_bbox.y1 + 0.005
    x_l_center = (ax_bbox.x0 + ax_bbox.x1) / 2.0

    # Convert longitude offset to an approximate horizontal figure position.
    # Larger l is placed to the left of the centre.
    x_l_extra = x_l_center - ((l_extra - l_center) / (2.0 * half_deg)) * (ax_bbox.x1 - ax_bbox.x0)

    fig_cur.text(x_l_extra, y_top, f"{l_extra:.1f}°",
                 ha="center", va="bottom", fontsize=9)

    fig_cur.text(x_l_center, y_top, f"{l_center:.1f}°",
                 ha="center", va="bottom", fontsize=9)

    # Left latitude labels.
    x_left = ax_bbox.x0 - 0.005
    y_b_center = (ax_bbox.y0 + ax_bbox.y1) / 2.0

    # Convert latitude offset to an approximate vertical figure position.
    # Smaller b is placed below the centre.
    y_b_extra = y_b_center + ((b_extra - b_center) / (2.0 * half_deg)) * (ax_bbox.y1 - ax_bbox.y0)

    fig_cur.text(x_left, y_b_extra, f"{b_extra:.1f}°",
                 ha="right", va="center", fontsize=9)

    fig_cur.text(x_left, y_b_center, f"{b_center:.1f}°",
                 ha="right", va="center", fontsize=9)

    # Axis names
    fig_cur.text((ax_bbox.x0 + ax_bbox.x1) / 2.0, y_top + 0.03,
                 r"$Galactic$ $l$ [deg]", ha="center", va="bottom", fontsize=14)

    fig_cur.text(x_left - 0.02, y_b_center + 0.2,
                 r"$Galactic$ $b$ [deg]", ha="center", va="center",
                 rotation=90, fontsize=14)
    # End of manual Galactic axis labels
    ...
    for iv in range(len(pas)):
        ...
        hp.projplot(ls, bs, c='g', lonlat=True, coord='G',
                    label='Robopol' if iv == 0 else None)
