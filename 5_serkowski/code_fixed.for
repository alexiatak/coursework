...
! INPUT
  ...
  211   format (1x, 2f6.3, f7.3) 
        ...
        if (test) then
          P (i) = tPm * exp ( -(aK * tlm + bK) * log(tlm / Lam (i))**2 )
        end if
...
! OUTPUT
  ...
  213 format (1x, A8, 
     ...
     &                 Pti/P_m0, P (i)/P_m0, sig_P (i)/P_m0,
     &                 Lam_m0/Lam(i)
 ...
 1000 stop
     ...
     &          COVAR(NCA,NCA),ALPHA(NCA,NCA),ATRY(MMAX),BETA(MMAX),
     &          DA(MMAX)
      IF(ALAMDA.LT.0.)THEN
        KK=MFIT+1
        DO 12 J=1,MA
