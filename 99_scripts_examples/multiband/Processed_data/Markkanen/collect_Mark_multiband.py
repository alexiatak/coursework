...
def outputAll(data):
    header = '#Name   JD   q        sq       u        su   x    y\n'

    OUT_PATH = os.path.join( os.getcwd(), 'Markkanen_for_notebook.dat' )
    fop_out = open(OUT_PATH,'w')
    fop_out.write(header)
    for i,obs in enumerate(data):
        ...
        out_str = "s"+str(i+1)+" = Obs('"+ obs.star +"', " + str(obs.jd) + ",  "
        out_str += "{:9.5f},".format(obs.q)
        out_str += "{:9.5f},".format(obs.sq)
        out_str += "{:9.5f},".format(obs.u)
        out_str += "{:9.5f},".format(obs.su)
        out_str += "{:7.1f},".format(x)
        out_str += "{:7.1f}".format(y)
        out_str += ")\n"
