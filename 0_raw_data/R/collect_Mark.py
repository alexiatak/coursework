...
def readData(star_name):
    ...
    for dr in dirs:
        ...
        for obs in data:
            ...
            base_star = obs.star.rstrip('ABCDEFGHIJKLMNOPQRSTUVWXYZ')                       #I want to read all series of one object
            fop = open(os.path.join(obs.full_fits_file, base_star + '_result.dat'))         #ex Mark_24,Mark_24B
...
def outputStar(data_individ_star):
    ...
    OUT_PATH = os.path.join( os.getcwd(), 'Markkanen', data_individ_star[0].star +'_raw.dat' )
...
def outputAll(data):
    ...
    OUT_PATH = os.path.join( os.getcwd(), 'Markkanen.dat' )
    fop_out = open(OUT_PATH,'w')
    fop_out.write(header)
    for obs in data:
        out_str = '{star: <16}'.format(star=obs.star)
