...
def readInstPol():
    ...
    for spn in range(len(years)):
        year = years[spn]
        with open('../../1_correct_instr/create_model/i/fit_Q'+year+'.pickle', 'rb') as f:
            instQ[year] = pickle.load(f)

        with open('../../1_correct_instr/create_model/i/fit_U'+year+'.pickle', 'rb') as f:
            instU[year] = pickle.load(f)

    return instQ, instU

def readEVPAZP():
    fop = open('../../1_correct_instr/create_model/i/EVPA_zp.txt')
    fop.readline()
    line = fop.readline()
    sl = line.rstrip('\n').split()
    fop.close()
...
def writeData(comb_stars):
    ...
    for path in ['Markkanen_final.dat', '../../0_data/I/Markkanen_final.dat']:
...
if __name__ == "__main__":
    print("#"*80)
    print("Working on 2025 Markkanen cloud data")
    print("#"*80)
